...
def _get_mode_instructions(mode: str, auto_confirmed: bool = False) -> str:
    ...
    if mode == "manual":
        return (
            "РЕЖИМ: MANUAL\n"
            "1. Сначала напиши как понял задачу (1-2 предложения)\n"
            "2. Спроси: \"Верно? Выполняю?\"\n"
            "3. ТОЛЬКО после подтверждения (\"да\", \"верно\") формируй [COMMAND_FOR_CODE]\n"
            "4. НЕ формируй команду сразу — сначала подтверждение!"
        )
    if auto_confirmed:
        return (
