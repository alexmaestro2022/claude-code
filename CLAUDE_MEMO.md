...
### Логика LONG:
1. **Тренд вверх**: EMA50 > EMA100 > EMA150, наклон положительный, EMA раздвинуты
2. **Откат**: цена уходит ниже EMA50
3. **Тест**: цена касается EMA100 как поддержки, не пробивает
4. **Запрет**: откат НЕ касается EMA150
5. **Вход**: первая свеча которая открылась выше EMA50
6. **SL**: под минимумом отката + буфер
7. **TP**: R:R 1.5 или трейлинг (BE + подтяжка на новых high)
...
- **Фильтр тренда**: наклон EMA > 0.1%, расстояние > 1x ATR
- **Касание EMA100**: допуск 0.1%
- **Запрет EMA150**: консервативный режим (вкл по умолчанию)
- **SL**: под/над откатом или фикс. %
