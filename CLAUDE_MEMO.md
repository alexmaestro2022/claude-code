...
### Назначение:
Детальное логирование и автоисправление при открытии сделок.
...
### Что логируется:
1. **Настройки бота** - плечо, размер, TP/SL режимы, маржа
2. **Отправлено на Bybit** - leverage, qty, takeProfit, stopLoss
3. **Проверка позиции** - сравнение ожидаемых и реальных параметров

### Автоисправление:
При расхождении параметров автоматически исправляются:
- ❌ Плечо не совпадает → `set_leverage()`
- ❌ Маржа не совпадает → `set_margin_mode()`
- ❌ TP/SL не выставлены → `update_take_profit()` / `update_stop_loss()`
- ⚠️ Размер неверный → только логирование (изменить нельзя)

### Формат лога:
```
[2026-01-21 15:30:00] TRADE OPEN BTCUSDT LONG

НАСТРОЙКИ БОТА:
- Плечо: 10x
- Размер: 10 USDT
- TP режим: risk_ratio (R:R 2)
- SL режим: supertrend (1%)
- Маржа: isolated
...
✅ Плечо: 10x — совпадает
❌ TP: отсутствовал → ИСПРАВЛЕНО, выставлен 51000
✅ SL: 49500 — совпадает
✅ Маржа: isolated — совпадает

ИТОГ: ✅ Позиция открыта корректно после исправлений
============================================================
```
