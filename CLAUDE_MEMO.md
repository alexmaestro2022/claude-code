...
- Виджет карточки (`updateApiUsageWidget()`): прогресс-бар бюджета, статистика (сегодня, неделя, вызовы, токены), остаток, **статистика по агентам** (топ-5 с % бюджета)

---

**Последнее обновление:** 2026-01-28 (fix: restore agent usage statistics in card)
