...
- `static/` - иконки, manifest.json для PWA, локальные JS/CSS (Tailwind, highlight.js, admin.css)
...
- **`_is_auto_confirmation()`** — word-boundary проверка: `re.findall(r'\w+')` + пересечение с confirm-словами (без подстрока-матча)
- Порог длины сообщения: 60 символов (короче = подтверждение, длиннее = новая задача)
- При Auto режиме: первое сообщение → Claude спрашивает план → пользователь подтверждает → далее автоматически
- Сброс `auto_confirmed` при: новой задаче (>60 символов), смене режима, новой сессии
- Follow-up промпт: 10 последних сообщений (было 5), обрезка до 800 символов (было 500)
...
- Новая задача: длинное сообщение (>60 символов) всегда сбрасывает `auto_confirmed`
...
- **Persistent admin sessions** — сессии и auth-коды сохраняются на диск и переживают рестарты бота:
  - Файлы: `/opt/aila/data/admin/sessions.json`, `/opt/aila/data/admin/auth_codes.json`
  - `_load_sessions()` / `_load_auth_codes()` — загрузка при старте (фильтрует expired)
  - `_save_sessions()` / `_save_auth_codes()` — сохранение при создании/удалении/обновлении
