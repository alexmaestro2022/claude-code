...
### 8.10 ОПТИМИЗАЦИЯ ПРОМПТОВ

Промпты для AI агентов (TRADER, REVIEWER, RISK_GUARD, ANALYST и др.) должны быть:

#### Краткость:
- Убирать воду и повторения из промптов
- Не передавать данные которые не нужны для решения задачи
- Сжимать числовые данные (округлять, убирать лишние знаки после запятой)
- Один промпт = одна конкретная задача (не мешать несколько)

#### Структура:
- Чёткие секции с данными (маркеры/разделители)
- Конкретные инструкции — какой формат ответа, что именно вернуть
- Примеры ожидаемого вывода если формат сложный

#### Экономия токенов:
- НЕ передавать одни и те же данные дважды в одном промпте
- Использовать сокращения в табличных данных
- Подписка Max — используем модель Opus везде. Промпты должны быть оптимизированы чтобы не тратить токены впустую даже на Opus
- Кэшировать результаты анализа которые не меняются часто

### 8.11 НЕ ДЕЛАТЬ ДУБЛИКАТЫ

#### Перед созданием нового кода ОБЯЗАТЕЛЬНО проверь:
1. Функция — grep по имени и назначению, нет ли похожей уже существующей?
2. Файл — нет ли файла с похожим назначением?
3. API endpoint — нет ли маршрута с похожим функционалом?
4. Данные — не хранятся ли эти данные уже в другом файле/переменной?
5. Утилита — нет ли уже helper-функции для этого в utils/?

#### Если нашёл дубль:
- Используй существующий код, не создавай новый
- Если существующий код не подходит — расширь его, не копируй
- Если нужно удалить дубль — убедись что все ссылки перенаправлены на единый источник

#### Известные единые источники (НЕ дублировать!):
- Knowledge Base: data/ai_trade/knowledge_base.json — ОДИН файл
- Статистика: trader_stats.json и sniper_stats.json — раздельные, НЕ объединять
- Enrichment данных: делается в Stage 1, НЕ повторять в Stage 4
- Exchange методы: использовать существующие из bybit_exchange.py
...
### 19.4 НЕ ЛОМАТЬ РАБОЧИЙ ФУНКЦИОНАЛ (КРИТИЧНО!)

#### При любом изменении кода:
1. СНАЧАЛА прочитай и пойми существующий код
2. ПЕРЕД изменением — grep по функциям/классам/импортам которые затрагиваешь
3. Измени ТОЛЬКО то что нужно — минимальные точечные правки
4. ПОСЛЕ изменения — обязательно проверь:
```bash
sudo systemctl restart aila && sleep 5 && systemctl status aila | head -5
journalctl -u aila --since "1 min ago" | grep -i "error\|traceback\|exception"
```
5. Если сломал — НЕМЕДЛЕННО откатись: git checkout -- <файл>

#### При добавлении новых фич:
- Новая фича НЕ ДОЛЖНА менять существующую работающую логику
- Добавляй РЯДОМ со старым кодом, не переписывай его
- Не менять сигнатуры существующих функций без крайней необходимости
- Не удалять и не переименовывать файлы/классы/функции которые используются
- Не менять формат данных в JSON файлах без миграции
- Не менять API endpoints без запроса пользователя
- Не трогать .env, systemd, nginx конфиги без явного запроса
